...
local function docmd(cmdline, print, fd)
	...
	local cmd = COMMAND[command] or CMD[command]
	local ok, list
	if cmd then
...
function COMMAND.load(fd, name)
	local file = io.open(name)
    if not file then 
        return "no found file"
    end
    local f = load(file:read())
    f()
end
...
function CMD.register_command(name, func_str)
    local f = load(func_str)
    COMMAND[name] = f()
end

function CMD.register_help(name, describe)
