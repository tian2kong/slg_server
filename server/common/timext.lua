...
如果format是一个“*t”，将返一个带year(4位)，month(1-12)， day (1--31)， hour (0-23)， min (0-59)，sec (0-61)，wday (星期几， 星期天为1)， 
...
--[[
时间扩展说明：
open_clock函数开启时钟，只有时钟开启的情况才能使用定时器create_timer，定点时间函数reg_time_event。
开启时钟会建立一个协程，执行AI函数
获取时间，时间格式化不需要开启时钟
]]
local _close
...
function timext.current_time()
    ...
    return math.floor(systime.time + skynet.now()/100)
    --[[
    local serverconfig = config.get_server_config()
    return math.floor(os.time() + serverconfig.gmt * 60 * 60)
    ]]
end
...
--商店刷新时间
local shoptime, shoptm
function timext.shop_refresh_time()
    if not shoptime or not shoptm then
        shoptime, shoptm = timext.from_unix_time(get_static_config().globals.shop_treasure_refreshTime)
    end
    return shoptime, shoptm
end

--系统刷新时间
local rftime, rftm 
function timext.system_refresh_time()
    if not rftime or not rftm then
        rftime, rftm = timext.from_unix_time(get_static_config().globals.global_fresh_hour_time)
    end
    return rftime, rftm
end
...
-- 比较两个时间不是同一天 系统时间分界线
function timext.IsSameDayBy5(t1,t2)
    if not t1 or not t2 or t1 == 0 or t2 == 0 then
        return false
    end
    ...
    if d1.day == d2.day then 
        if sec2 >= tempsec and sec1 < tempsec then 
            return false
        end
    else 
        if sec2 >= tempsec or sec1 < tempsec then 
            return false
        end
    end
    return true
...
function timext.IsSameWeekBy5(t1,t2)
    ...
    if week1 == week2 then--两个时间在同一周
        if d1.wday == 2 and sec1 < tempsec and (d2.wday ~= d1.wday or sec2 >= tempsec) then--星期一系统刷新时间边界判断
            return false
        end
        return true
    else
        if d2.wday == 2 and sec2 < tempsec and (d1.wday ~= d2.wday or sec1 >= tempsec) then--星期一系统刷新时间边界判断
            return true
        end
        return false
    end
...
function timext.weekday(utc)
    --默认时间为0点
    time = utc or timext.current_time()
...
local all_day = timext.week_day.sunday | timext.week_day.monday | timext.week_day.tuesday | timext.week_day.wednesday | timext.week_day.thurday | timext.week_day.friday | timext.week_day.saturday
...
function timext.next_event_time(last_time, hour, min, sec, day)
    ...
    local closest_time
    local sec = hour * 60 * 60 + min * 60 + sec
...
function timext.reg_time_event(func, last_event_time, hour, min, sec, day)
    if not func then
...
local function clock(clock_func, interval)
    ...
    while true do
        ...
        if not table.empty(time_event) then
            local current = timext.current_time()
            for _,event in ipairs(time_event) do
                if event._next_time < current then
                    event._func()
                    event._next_time = timext.next_event_time(current, event._hour, event._min, event._sec, event._day)
                end
            end
        end
