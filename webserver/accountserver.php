<?php 
	...
	if ($code == 0) {
		$ret = false;
		$server = null;
		if ($invation && strlen($invation) > 0) {
			$serverid = floor(intval($invation) / PLAYER_SECTION);
			if ($serverid) {
				$server = getServerCfg($serverid);
				if ($server) {
					$ret = getServerMsg($server, $result['account']);
				}
			}
		} else {
		...
		if (!$ret) {
			...
			if ($server) {
				...
				$url=sprintf('/facebookinvate?account=%s&invatecode=%s', $result['account'], $invation);
				file_get_contents($host . $url);
			}
		}
	}
	$ret['code'] = $code;
